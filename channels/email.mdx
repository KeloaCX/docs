...
After saving, Keloa shows your workspace's `inbound_address`, e.g. `acme-x9k2pq@in.keloa.ai`. **This is the address you forward inbound mail to.** It's shared across every email channel in the workspace, so you only set up one forwarding rule per real mailbox.
...
You can add as many email channels as your plan allows.

| Plan | Email channels | Email domains |
| --- | --- | --- |
...
<Note>
Email is a **Growth+ feature**. Starter workspaces are widget-only — the email tile in **Settings → Channels** shows an upgrade prompt.
</Note>

Common setups:

- `support@` for general inquiries, `billing@` for invoices, `careers@` for hiring — three email channels, one verified domain, one inbound alias, three forwarding rules. This fits within Growth's 3-address / 1-domain limit.
- Multiple brands on different verified domains — `support@brand-a.com`, `hi@brand-b.com` — needs more than one email domain, so Business (3 domains) or Scale. Every channel still routes through the same inbound alias.
...
| Issue | Fix |
| --- | --- |
| Mails not arriving | Send a test directly to your `inbound_address` (e.g. `acme-x9k2pq@in.keloa.ai`). If that works, the forwarding rule is the issue. |
...
| `From` shows "via keloa.ai" in Gmail | Expected when only DKIM is set up but `Return-Path` alignment is missing. Verify SPF too. |
